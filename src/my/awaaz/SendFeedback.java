...
public class SendFeedback extends JPanel {
   ...
   */
  public SendFeedback(int stars, float rating) {
    this(stars, rating, 0);
  }
   ...
   */
  public SendFeedback(int stars, float rating, int selection) {
    this.stars = stars;
    this.rating = rating;
    ...
    addMouseListener(new MouseAdapter() {
      ...
      public void mousePressed(MouseEvent event) {
        if (isEnabled()) {
          ...
          SendFeedback.this.selection = 1 + (event.getX() / STAR_BACKGROUND_IMAGE.getWidth(null));  
          for (int i = 0; i < listeners.size(); i++) {
            listeners.get(i).handleSelection(SendFeedback.this.selection);
          }
          repaint();
        }
      }
      public void mouseReleased(MouseEvent event) {
        if (isEnabled()) {
          if (!done) {
            rollover = 1 + (event.getX() / STAR_BACKGROUND_IMAGE.getWidth(null));        
            repaint();  //used for operations other than paint(like color change,etc)
          }
        }
      }
    });
   ...
   * 1. Load image via File into a byte array
   * 2. Then use SOP() to print single bytes out
   * 3. Copy/Paste o/p into program
   */
